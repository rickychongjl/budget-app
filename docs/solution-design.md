...
- **Service worker:** precaches the app shell; runtime-caches `GET /api/me` only, network-first, so the app can open with no signal. (M8 decision: it was to cache `/api/cycles/*` stale-while-revalidate too, but by then every answer was already kept whole in IndexedDB per user, stamped with when the server gave it, and a worker-served copy would carry a fresh stamp for a stale answer and break the offline overlay.) Never caches `/auth/*`, and never serves a page for `/api/*`, `/auth/*` or `/health`. A new build is offered, not applied unasked.
