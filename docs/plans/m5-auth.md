...
- `GET /auth/csrf` (global rate limit only, not the strict `/auth` policy): `IAntiforgery.GetAndStoreTokens` for whoever the caller is now; the cookie token stays `HttpOnly` (`Secure` as the session cookie, `SameSite=Strict`), the request token is returned in the body as `{ "token": ... }` and the page keeps it in memory. No script-readable cookie (changed after CodeQL flagged `HttpOnly = false`, 2026-09-20). `200`.
