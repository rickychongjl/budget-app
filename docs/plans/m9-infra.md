...
- **Log alerts every fifteen minutes, not five.** A failure rate and a P95 are not platform metrics, so they are scheduled query rules, priced by frequency; fifteen is the cheapest tier. The two job alerts are metric alerts on `Executions` with `state = Failed`, hourly.
