...
- **A dev-only `/kit` page** (not in the first draft; offered on 2026-09-20 and built on the assumption of a yes, since the reply was "continue on". Deleting `src/dev/` and the three lines in `App.tsx` removes it): every `ui/` component in every state with a theme toggle, to run MASTER section 14 against before screens depend on the kit. `npm run dev`, then `/kit`. It is behind `import.meta.env.DEV`, and a check that the string "UI kit" is absent from `dist` confirmed the production build drops it.
