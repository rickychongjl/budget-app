...
All fourteen slices are in. Web: 280 tests (Vitest), `oxlint` clean, `tsc -b` and `vite build` green. .NET: 314 tests (Domain 119, Application 82, Infrastructure 34, API 79), `dotnet build -warnaserror` clean.

Checked against a real stack, not only mocks: this branch's production image on its own port, a throwaway SQL container, `migrate` seeding the demo, and headless Edge driven over the DevTools protocol at 390px (360 and 700 for overflow). Against the **built SPA served by the API**:

- "Try the demo" fetched the token, got the cookie and rendered the seeded Home, in light and dark.
- Add $12.50 to Groceries online: $242.90 became $255.40 with a "Saved" toast and Undo.
- Network off (browser emulation): two more adds showed at once ($362.40) and the banner read "Offline. 2 changes waiting to sync." Network on: the banner cleared with no flicker in the number; after a reload the server had $362.40 and the outbox held 0 rows.
- A deep link to a category's transactions survived a reload (the `index.html` fallback), with the right title and rows.
- Onboarding, with the demo emptied in the throwaway database: empty Home, draft from a start date and opening balance (an empty balance moved focus to the field with its message), two categories added, Confirm, and Home at "day 1 of 30".
- No sideways scroll and no tap target under 44px on any screen measured.

Not checked, and why: a real phone (safe-area insets, haptics, the iOS date and select pickers, standalone mode) and a screen reader. Focus trap, Escape and focus return on sheets and dialogs are the browser's `<dialog>` behaviour and have no automated test until the M8 Playwright specs. The Entra button and the refused-sign-in screen were seen only through tests and a direct visit to `/signin?error=auth.not-allowed`, because the local stack has no Entra settings.

Where the code differs from the plan below:

- **Onboarding asks for the start date and opening balance first, then categories** (the plan, like the stories, listed categories first). A category can only be added to a cycle that exists, so the Draft has to be created before its categories. It is one screen with three numbered sections; the later two appear once the Draft exists, and the Draft is saved on the server at each step so leaving and returning resumes.
- **A user with no cycle sees Home's empty state with one action, not a redirect** to `/onboarding`. It is MASTER's own empty state ("No budget yet. Set up your first cycle.") and it satisfies story "Onboarding 1" (no existing data) more plainly than being moved somewhere.
- **The cycles list has no "spent against budgeted" yet** (MASTER 9, "Cycle list row"). `GET /api/cycles` returns cycles without totals; the endpoint that has them, `GET /api/reports/cycles`, is M7's. The rows show dates, the status badge and money accrued.
- **Money accrued in the list is `closing - opening`, worked out in the page** from the two balances the list already has. The detail page uses the API's `accrued`.
- **Reordering renumbers every category in the cycle** (one queued `category.edit` each, at most 20). The rollup arrives in sort order but without the numbers, so renumbering all of them is what makes a move exact whatever was stored. Marked `ponytail:`; send only the changed rows once the API returns `sortOrder`.
- **A row the server has accepted stays in the overlay until a snapshot newer than the sync arrives** (`syncedAt` on the outbox row, compared with the query's `dataUpdatedAt`). Not in the plan: deleting it at the moment of the sync made a saved transaction vanish until the refetch landed, and keeping it past the refetch counted it twice.
- **An edit or delete is queued with what the transaction looked like before** (`before`: cycle, category, amount; local only, never sent). That is how the overlay adjusts a category's total without holding the transaction list, and how the closing-balance prompt knows which cycle an edit belonged to.
- **Undo of an add that has not been sent cancels the pair locally** and sends nothing; once the create is on the wire or accepted, the delete is queued like any other.
- **A deleted transaction has no Undo.** The confirm dialog is the protection; re-creating with the same `ClientId` after a delete is possible but was not needed.
- **A reversal (negative amount) keeps its sign when edited but cannot be entered**: the keypad has no minus key (MASTER's keypad is digits, point and backspace). Marked `ponytail:`.
- **The closing-balance prompt appears when the change syncs**, which for an offline change is when the connection returns, because only the server says whether a write landed in a past cycle.
- **`Screen` has a `back` option** for screens reached from another screen (a category's transactions, a cycle, categories), so the edge swipe is not the only way back.
- **`[hidden] { display: none !important }` is global.** A component's own `display: grid` otherwise beats the browser's rule and shows what was meant to be hidden.
- **Two traps found by running on Windows and by measuring**, both now rules in CLAUDE.md: `Keypad.tsx` beside `keypad.ts` resolved to the same file on a case-insensitive disk (renamed to `amountInput.ts`); and a 2px selection border around a hidden radio shrank the colour and icon targets to 40px (the ring is an inset shadow now).
...
- **A dev-only `/kit` page** (not in the first draft; built on an assumed yes and kept after review on 2026-09-20: "kit looks very good!"): every `ui/` component in every state with a theme toggle, to run MASTER section 14 against before screens depend on the kit. `npm run dev`, then `/kit`. It is behind `import.meta.env.DEV`, and a check that the string "UI kit" is absent from `dist` confirmed the production build drops it.
...
- **The outbox-by-user rule landed in slice 7, not slice 4**, with the outbox itself: sign-out clears the query cache, the Dexie cache and the token and keeps the queue; `configureOutbox` discards any queue and cache that is not the signed-in user's.
...
- **Red-first, honestly:** every test file was written and run before its code and failed on the missing import. A real failing assertion before the fix was seen for the overlay (a float gave 102.49999999999999, so percentages are rounded), the outbox (the undo-collapse guard was too coarse: "a drain is running" became "this row is on the wire"), the transaction list (unsorted with an empty outbox), the add sheet (two `Sheet` instances, so the dialog was re-created when data arrived and would have replayed its slide-up on every open) and the API host (slice 1). The rest passed on the first run after the code was written.
